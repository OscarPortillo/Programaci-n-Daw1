...
public class GestionLibroAutor {
	public static void main(String[] args) {
		...
		posAutor=creaAutores(autores, comentarios, numComentable);
		posLibro=creaLibros(autores,libros, comentarios, 3);
		System.out.println("******************************AUTORES******************************");
		for (int i=0; i<comentarios.length;i++){
			if(comentarios[i] instanceof Autor){
				if (comentarios[i]!=null ){//SI NO SON NULL ENTRA
					Leer.mostrarEnPantalla(comentarios[i].comentar());//MOSTRARÁ SOLO EL METODO COMENTAR DE AUTOR
				}
			}
		}
		System.out.println();
		...
		for (int i=0; i<comentarios.length;i++){
			if(comentarios[i] instanceof Libro){				
				if (comentarios[i]!=null ){
					Leer.mostrarEnPantalla(comentarios[i].comentar());//MOSTRARÁ EL METODO COMENTAR DE LIBROS
				}
			}
		}
		...
		do {
			...
			case 7:
				if(posLibro==0){
					...
					listarLibros(libros);
					titulo = Leer.pedirCadena("Titulo del libro a modificar.");
						cambiarReseña(libros, titulo, autores);
				}
				break;
			}
		} while (opcion != 0);
	...
	public static void cambiarReseña(Libro [] libros, String titulo, Autor [] autores){
		@SuppressWarnings("unused")
		...
		if(indiceLibro != -1){
			...
			reseña=Leer.pedirCadena("Introduce la nueva reseña");
			libro.setResena(reseña);
		}else{
	...
	private static Integer menu() {
		...
		Leer.mostrarEnPantalla("\n1- Crear autor.\n2- Crear libro.\n3- Modificar autor.\n" + "4- Modificar libro.\n" + "5- Listado de autores.\n"
				+ "6- Listado de libros.\n7 Cambiar la reseña de un libro.\n" + "0- Salir.");
	...
	private static Integer creaAutor(Autor[] autores, Integer posAutor){
		Autor autor;
		String nombreAutor, email, sexo,bio;
		Character letraSexo;
		Integer indiceAutor;
		...
		if (indiceAutor == -1) {
			...
			bio=Leer.pedirCadena("Biografia del autor?");
			autor = new Autor(nombreAutor, email, letraSexo,bio);
	...
	public static Integer crearLibros(Integer posLibros, Libro[] libros, Autor[] autores) {
		...
		if (indiceAutor == -1) {
			...
			reseña=Leer.pedirCadena("La reseña del libro?");
			libros[posLibros] = new Libro(titulo, autor, precio, cantidad,reseña);
			posLibros++;
	...
	public static Integer creaAutores(Autor []autores, Comentable[] comentarios, Integer numComentable){
		// creamos 3 autores
		autores[0]= new Autor("Rosa Montero", "rmontero@gmail.com", 'f',"Yo soy Rosa Montero");
		comentarios[numComentable]=autores[0];
		numComentable++;
		autores[1]= new Autor("Juan Jose Millas", "jjmillas@hotmail.es",'m',"Yo soy Juan Jose Millas");
		comentarios[numComentable]=autores[1];
		numComentable++;
		autores[2]= new Autor("Almudena Grandes", "ag@gmail.com",'f',"Yo soy Almudena Grandes");
		comentarios[numComentable]=autores[2];
		numComentable++;
		return 3;
	}// crea 3 autores

	public static Integer creaLibros(Autor[] autores, Libro[] libros, Comentable[] comentarios, Integer numComentable){
		// creamos 5 libros
		libros[0]= new Libro("Lágrimas en la lluvia", autores[0],19.95 ,100000,"El cielo se puso a llorar");
		comentarios[numComentable]=libros[0];
		numComentable++;
		libros[1]= new Libro("La ridícula idea de no volver a verte", autores[0], 15., 200000,"No wey no nos vamos a ver");
		comentarios[numComentable]=libros[1];
		numComentable++;		
		libros[2]= new Libro("Desde la sombra", autores[1], 25., 75000,"Y no desde donde da el sol LOL");
		comentarios[numComentable]=libros[2];
		numComentable++;
		libros[3]= new Libro("Malena es un nombre de tango", autores[2], 22., 100000,"Y feo ese nombre hp");
		comentarios[numComentable]=libros[3];
		numComentable++;		
		libros[4]= new Libro("Las edades de Lulú", autores[2], 30., 200000,"De la era de piedra");
