...
public class Principal {
		...
		for(int i=0;i<coche.length;i++){
			if(coche[i]instanceof Normal){
				...
				if(nor.isDisponible()==true){
					System.out.println("numero "+i+" "+nor.verCoche());
				}
			}
	...
	public static void verOcupados(Coche coche[]){
		for(int i=0;i<coche.length;i++){
			if(coche[i]instanceof Normal){
				Normal nor;
				nor=(Normal) coche[i];
				if(nor.isDisponible()==false){
					System.out.println("numero "+i+" "+nor.verCoche());
				}
			}
			if(coche[i]instanceof Lujo){
	...
	public static void crearCoches(Coche coche[]){
		...
		String lujo[]={ "Jaguar", "Lamborghini", "Audi", "BMW","Koenigsegg","Ranault","Ferrari","Porsche"};
		String normal[]={"Opel", "Ford", "Chevrolet", "Nissan", "BMW", "Toyota", "Suzuki", "Citroen","Mini", 
				"Jeep", "Mazda","Ranault","Fiat","Kia","Hiundai","Honda"};
		String modelo[]={"Modelo1A", "Modelo2A", "Modelo3A", "Modelo4A", "Modelo5A", "Modelo6A",
				"Modelo1B", "Modelo2B", "Modelo3B", "Modelo4B", "Modelo5B", "Modelo6B","ModeloNBe","MoEEEE","JAJAJAJA"};
		int num=0;
		for(int i=0;i<15;i++){
			coche[i]=new Normal(matricula[i],normal[i],modelo[i],fechaRandom(),true,aparcamiento());

		}
		for(int i=15;i<coche.length;i++){
